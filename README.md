...
Now Playing adapts seamlessly to how you navigate your PC, offering two dedicated control modes:

### Quick Access Menu (QAM)
**Instant control without leaving your game.**
Open the Quick Access Menu to check the active track, artist, and album at a glance. Enjoy responsive layouts, glowing animated artwork that reacts to your music, service-specific accent colors, and custom navigation grids tailored to your active audio source.

### Big Picture Mode
**An immersive, TV-optimized browser.**
Dive into a dedicated, full-screen music experience designed for gamepads. Explore artist discographies, browse custom playlists, manage playback queues, and get lost in rich visualizers without ever reaching for a keyboard or mouse.
...
### Interface Readability & Smart Polish
*   **Instant Track Transitions:** Song titles and album covers swap immediately across every source just like local files, while the progress bar waits until real audio begins playing.
*   **Smart Text Contrast:** Song title, artist, and album text automatically turn dark (`#1A1A1A`) when displayed over light or pastel covers such as yellow, cyan, green, and pink across all music sources.
*   **Gamepad-Friendly Settings:** Selectors for fullscreen effects and 3D resolution use plain stacked buttons with clean checkmarks on active options, ensuring reliable controller navigation in both QAM and Big Picture.
*   **Clean Source Switching:** Switching between services resets the display to a clean state so local music never displays leftover tracks from YouTube Music or Spotify.
*   **Multilingual Support:** Professionally localized interface translations available in 10 languages.

### Per-Source Volume Control & Smart Switching
Adjust your music volume independently without changing your main Windows master volume or your game audio. Now Playing remembers custom volume levels for every single service. When you change your music source, the plugin automatically launches the new application and cleanly closes the previous one to save system resources and prevent audio overlap.

## Setup Guide

### Your Music (Local Files)
1. Open Now Playing Settings and select **Your Music**.
2. Add one or more music folders from your PC.
...
4. Save the app, copy your **Client ID**, and paste it into Now Playing Settings in Steam. No Client Secret is required.

### YouTube Music
1. In Now Playing Settings, select **Connect YouTube Music**.
2. A secure, temporary browser window will appear above Steam. Log into your Google account once.
3. Close the window when finished. The plugin captures your session cookie automatically without requiring any Google Cloud configuration.

## Troubleshooting & Known Tips

*   **YouTube Music feels sluggish?** A VPN can trigger YouTube bot checks and slow down track resolution. Try disabling your VPN if loading feels slow.
...
*   **Album cover shortcuts:** Clicking the YouTube Music cover only opens the album page when the playing track carries an explicit album ID.
...
*   **Spotify 5 GB cache:** Taking full advantage of the 5 GB audio cache requires rebuilding the bundled `SpotifyPlaybackBridge` binary on Windows.
*   **Interface feeling slow?** Click **Restart plugin services** in Settings to reset background helpers, clear shared memory, and reconnect audio bridges immediately.

## Installation

Download **`Now-Playing-decky_Installer-2.2.0.zip`** and install it directly through Decky Loader by enabling Developer Mode and selecting **Install from zip**.
